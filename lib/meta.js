...
// 5b) Perfil ampliado: incluye la biografía si el permiso lo permite.
// Si falla (algunos permisos no dan biography), cae al perfil básico.
export async function getProfileRich(token) {
  const fields = "user_id,username,account_type,followers_count,media_count,profile_picture_url,biography,name,website";
  const r = await fetch(`${IG_GRAPH}/me?fields=${fields}&access_token=${token}`);
  const data = await r.json();
  if (data.error) return getProfile(token);
  return data;
}

// 5c) Publicaciones recientes (para deducir el nicho y el tono de la cuenta)
export async function getRecentMedia(token, limit = 12) {
  const fields = "caption,media_type,like_count,comments_count,timestamp,permalink";
  const r = await fetch(`${IG_GRAPH}/me/media?fields=${fields}&limit=${limit}&access_token=${token}`);
  const data = await r.json();
  if (data.error) throw new Error(data.error.message);
  return data.data || [];
}
